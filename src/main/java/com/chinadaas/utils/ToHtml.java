...
public class ToHtml {
    ...
    public void printPage() throws IOException {
        try {
            ...
            if (completeHTML) {
                ...
                out.format("<script src=\"http://code.jquery.com/ui/1.10.3/jquery-ui.js\"></script>%n");
                out.format("<script>%n");
                out.format("$(document).ready(function() {$(\"#main\").tabs(); });");
                out.format("</script>%n");
                out.format("</head>%n");
                out.format("<body>%n");
            }
    ...
    private void printSheets() {
        ...
        {
        	try{
        		Sheet sheet = wb.getSheetAt(itr);
        		out.format("<div id=\"tabs-"+(itr+1)+"\">%n");
        		printSheet(sheet);
        		out.format("</div>%n");
        		itr++;
